...
def main(args):
    ...
    train_primary_path = data_conf_dir / "train_primary.yaml"
    if not (train_primary_path).exists():
        raise ValueError(f"File {train_primary_path} does not exist. Please run the `populate_data_conf.py` script first.")

    with open(train_primary_path, "r") as fout:
        ...
        with open(data_conf_dir / "included_corpora.yaml", "w") as fout:
