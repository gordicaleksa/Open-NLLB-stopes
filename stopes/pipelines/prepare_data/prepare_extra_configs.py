...
yaml_config_root_path = pathlib.Path(__file__).parent
...
def prepare_configs(args):
    ...
    with open(train_primary_config_path, "r") as fout:
        ...
        with open(yaml_config_root_path / "train_corpora.yaml", "w") as fout:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_primary_config_path",
        type=str,
        required=True,
        help="Path to train_primary.yaml config (output of populate_data_conf.py script)",
    )
